                    ...
                    if 'messages' in value:
                        for message in value['messages']:
                            if message['type'] == 'text':
                                ...
                                # Handle the message
                                handle_message(client, Message(text=text, from_user=from_id))
...
def send_welcome_message(client: WhatsApp, to: str):
    ...
    try:
        response = client.send_message(to=to, text="Merhaba! Nasıl yardımcı olabiliriz?")
...
def handle_message(client: WhatsApp, message: Message):
    ...
    if from_id not in users_greeted:
        ...
        return  # Exit the function after sending the welcome message
...
async def global_exception_handler(request, exc):
    ...
    return JSONResponse(status_code=500, content={"message": "An unexpected error occurred.", "error": str(exc)})